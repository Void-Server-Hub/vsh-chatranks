...
import { COLORS } from '../../Other/Colors/ColorsConstant.js';
...
export function getColor(player, tagseparator, message) {
	...
	if (!chatColorTag) {
		// Default behavior when no color tag found
		switch (tagseparator) {
	...
	// Handling custom color and gradient cases
	...
	if (colorMap[colorKey]) {
		...
		// Handle gradients and special effects like Rainbow
		return otherSymbols + new CustomText(message).setGradient(colorKey);
	} else {
		// If the color tag is not recognized, fall back to default
